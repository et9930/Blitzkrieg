...
{
	//ScrollBar обрабатывает NOTIFY сообщени¤ от детей
...
void CUISlider::Visit( interface ISceneVisitor *pVisitor )
{
	...
	if ( m_nMin < m_nMax )
	{
		...
		// здесь предполагаетс¤ что     nMin <= nPosition <= nMax
		SGFXRect2 rect;
...
void CUIScrollBar::Reposition( const CTRect<float> &rcParent )
{
	...
	//теперь ручками изменю позиции дл¤ составл¤ющих ScrollBar
	if ( !IsVertical() )
	{
...
{
	//провер¤ю, вдруг min или max кнопки в нажатом подсосто¤нии
